...
public class VoterController {
	...
	@GetMapping
	ResponseEntity<List<VoterDto>> showVoters() {
		return ResponseEntity.ok(voterFacade.showVoters());
	}

	@PostMapping
	ResponseEntity<?> addVoter(@RequestBody VoterDto voterDto) {
		voterFacade.addVoter(voterDto);
		return ResponseEntity.ok(HttpStatus.CREATED);
	}

	@PostMapping("vote")
	ResponseEntity<?> vote(@RequestBody UpdateVoterDto updateVoterDto) {
